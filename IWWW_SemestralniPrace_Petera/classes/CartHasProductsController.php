...
{
    static function insertOrUpdate($conn, $cart_id, $product_id, $quantity, $attributes)
    {
        $stmt = $conn->prepare("SELECT * FROM cart_has_products WHERE cart_id = :cart_id AND product_id = :product_id AND 
        ...
        if ($stmt->rowCount() == 0) {
            $stmt = $conn->prepare("INSERT INTO cart_has_products (cart_id, product_id, quantity, attributes) VALUES (:cart_id, :product_id, :quantity, :attributes)");
